...
#else
	//Allocation failure can be occur where there are a lot of thread contention
	//To avoid allocation failure, finite loop is changed with infinite loop
	//<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
	//for (i = 0; i < 10; i++) {
	//>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
	while (1) {
...
item *do_item_get(const char *key, const size_t nkey, const uint32_t hv, conn *c, const bool do_update) {
				...
				if (settings.lru_segmented) {
					if ((it->it_flags & ITEM_ACTIVE) == 0) {
						if ((it->it_flags & ITEM_FETCHED) == 0) {
							...
							} else if (c != NULL && !lru_bump_async(c->thread->lru_bump_buf, it, hv)) {
	...
	if (it != NULL || do_update == DONT_UPDATE) {
		if (c != NULL) {
...
static int lru_pull_tail(const int orig_id, const int cur_lru,
	...
	if (it != NULL) {
        ...
        if (move_to_lru) {
			...
			if ((flags & LRU_PULL_EVICT) && orig_id != 1) {
				uint32_t hv = hash(ITEM_key(it), it->nkey);
				item *rescue = do_item_alloc(ITEM_key(it), 
						it->nkey, 0, 0, 2);
				if (rescue != NULL) {
					rescue->skey = it->skey;
					rescue->val_ptr.tag = it->val_ptr.tag;
					rescue->val_ptr.vptr = it->val_ptr.vptr;
					memcpy(ITEM_data(rescue), "\r\n", rescue->nbytes);
					do_item_link(rescue, hv);
					do_item_remove(rescue);
				}
			}
		}
#else
...
bool lru_bump_async(lru_bump_buf *b, item *it, uint32_t hv) {
    bool ret = true;
    refcount_incr(it);
