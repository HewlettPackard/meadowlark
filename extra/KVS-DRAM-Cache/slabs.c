...
//>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
	size_t j;
	// Slabs for short cut are preallocated
	// In current implementation, preallocated slabs 
	// for short cut is 50% of total cache size
	for (j = 0; j < ((limit/(1024*1024UL)))/2; j++) {
		if (do_slabs_newslab(1) == 0) {
...
//>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
//This function added for checking slabs allocation status
void slabs_print() {
	int i;
	for (i = POWER_SMALLEST; i < MAX_NUMBER_OF_SLAB_CLASSES; i++) {
		printf("slabclass[%d].size = %u\n", i, slabclass[i].size);
		printf("slabclass[%d].sl_curr = %u\n", i, slabclass[i].sl_curr);
		printf("slabclass[%d].slabs = %u\n", i, slabclass[i].slabs);
	}
}
//>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
