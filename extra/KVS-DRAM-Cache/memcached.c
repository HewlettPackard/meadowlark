...
void conn_to_str(const conn *c, char *buf);
...
int add_chunked_item_iovs(conn *c, item *it, int len);
 ...
 */
volatile bool allow_new_conns = true;
struct event maxconnsevent;
void maxconns_handler(const int fd, const short which, void *arg) {
...
void complete_update_bin(conn *c) {
...
void process_bin_get_or_touch(conn *c) {
	item *it;
...
void append_bin_stats(const char *key, const uint16_t klen,
...
void append_ascii_stats(const char *key, const uint16_t klen,
		const char *val, const uint32_t vlen,
		conn *c) {
...
bool grow_stats_buf(conn *c, size_t needed) {
...
void handle_binary_protocol_error(conn *c) {
...
void bin_list_sasl_mechs(conn *c) {
...
void process_bin_sasl_auth(conn *c) {
...
void process_bin_complete_sasl_auth(conn *c) {
	assert(settings.sasl);
	const char *out = NULL;
...
void process_bin_update(conn *c) {
	char *key;
	int nkey;
	int vlen;
	item *it;
...
void process_bin_append_prepend(conn *c) {
	char *key;
	int nkey;
	int vlen;
	item *it;
...
void process_bin_delete(conn *c) {
	item *it;
...
int _store_item_copy_chunks(item *d_it, item *s_it, const int len) {
...
int _store_item_copy_data(int comm, item *old_it, item *new_it, item *add_it) {
				...
				|| comm == NREAD_APPEND || comm == NREAD_PREPEND))
	{
		...
		if (stored == NOT_STORED && failed_alloc == 0) {
			if (old_it != NULL)
				...
				do_item_link(it, hv);	//SEKWON: Insert to the hash table
...
inline bool set_noreply_maybe(conn *c, token_t *tokens, size_t ntokens)
{
...
void process_stat(conn *c, token_t *tokens, const size_t ntokens) {
...
inline int make_ascii_get_suffix(char *suffix, item *it, bool return_cas) {
	char *p = suffix;
	if (!settings.inline_ascii_response) {
		*p = ' ';
		p++;
...
inline item* limited_get(char *key, size_t nkey, conn *c) {
...
void process_touch_command(conn *c, token_t *tokens, const size_t ntokens) {
	char *key;
	size_t nkey;
	int32_t exptime_int = 0;
	item *it;
...
void process_arithmetic_command(conn *c, token_t *tokens, const size_t ntokens, const bool incr) {
...
void process_delete_command(conn *c, token_t *tokens, const size_t ntokens) {
	char *key;
	size_t nkey;
	item *it;
	...
	if (it) {
		...
		//>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
#else
		//This codes are added for covering the case where 
		//cached data was not found but, need to delete KVS data
		int ret;
		ret = item_unlink_kvs(key, nkey);
		if (ret == 0)
			out_string(c, "DELETED");
		else
			out_string(c, "NOT_FOUND");
...
void process_slabs_automove_command(conn *c, token_t *tokens, const size_t ntokens) {
	unsigned int level;
	double ratio;
...
void process_watch_command(conn *c, token_t *tokens, const size_t ntokens) {
	uint16_t f = 0;
	int x;
	assert(c != NULL);
...
void process_memlimit_command(conn *c, token_t *tokens, const size_t ntokens) {
	uint32_t memlimit;
	assert(c != NULL);
...
void process_lru_command(conn *c, token_t *tokens, const size_t ntokens) {
	uint32_t pct_hot;
	uint32_t pct_warm;
	double hot_factor;
	int32_t ttl;
...
bool _parse_slab_sizes(char *s, uint32_t *slab_sizes) {
	char *b = NULL;
	uint32_t size = 0;
	int i = 0;
...
int main (int argc, char **argv) {
	...
	conn c_obj;
	conn *my_c = &c_obj;
	my_c->thread = calloc(1, sizeof(LIBEVENT_THREAD));
	my_c->thread->suffix_cache = cache_create("suffix", SUFFIX_SIZE,
			sizeof(char*), NULL, NULL);
	my_c->thread->l = logger_create();
	my_c->thread->lru_bump_buf = item_lru_bump_buf_create();

	char key[100] = "hello";
	char value[100] = "world";
	item *kv = item_alloc(key, 6, 0, realtime(0), 6);
	memcpy(ITEM_data(kv), value, 6);
	enum store_item_type ret = store_item(kv, NREAD_SET, my_c);
	...
	switch (ret) {
		...
		case EXISTS:
			...
			break;
		case NOT_FOUND:
			printf("PUT: NOT_FOUND\n");
			break;
		case NOT_STORED:
			printf("PUT: NOT_STORED\n");
			break;
		default:
			printf("PUT: SERVER_ERROR Unhandled storage type.\n");
	}

	item *res = item_get(key, 6, my_c, DO_UPDATE);
	if(res) {
		printf("GET:\n");
		printf("key: %s\n", key);
		printf("value: %s\n", ITEM_data(res));
