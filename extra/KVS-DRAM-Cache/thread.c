...
void pause_threads(enum pause_thread_types type) {
	...
	//We configured the number of threads with 0
	//The number of threads for local mode is decided 
	//by thread_num configured by our application
 ...
 * needed.
 */
void item_remove_internal(item *item) {
    uint32_t hv;
    hv = hash(ITEM_key(item), item->nkey);
 ...
 */
void item_unlink_internal(item *item) {
    uint32_t hv;
    hv = hash(ITEM_key(item), item->nkey);
    item_lock(hv);
...
{	
	return store_item_internal(item, comm, c);
}
*/
/******************************* GLOBAL STATS ******************************/
