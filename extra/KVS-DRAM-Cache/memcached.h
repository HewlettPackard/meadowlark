...
#if CACHE_MODE == 1 || CACHE_MODE == 2 || CACHE_MODE == 3
//>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
typedef struct TagPtr TagPtr;
struct TagPtr {
	uint64_t tag;
	uint64_t vptr;
};
...
#if CACHE_MODE == 1 || CACHE_MODE == 2 || CACHE_MODE == 3
	//>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
	/* these variables are for short-cut scheme */
	uint64_t		skey;		/* short-cut */
	TagPtr			val_ptr;	/* version number, value pointer pointing to FAM area */
	//>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
#endif
...
int start_conn_timeout_thread(void);
...
bool set_noreply_maybe(conn *c, token_t *tokens, size_t ntokens);
...
int make_ascii_get_suffix(char *suffix, item *it, bool return_cas);
item* limited_get(char *key, size_t nkey, conn *c);
...
item *item_get_internal(const char *key, const size_t nkey, conn *c, const bool do_update);
item *item_touch(const char *key, const size_t nkey, uint32_t exptime, conn *c);
int   item_link(item *it);
void  item_remove_internal(item *it);
int   item_replace(item *it, item *new_it, const uint32_t hv);
void  item_unlink_internal(item *it);
...
enum store_item_type store_item_internal(item *item, int comm, conn *c);
