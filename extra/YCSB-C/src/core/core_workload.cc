...
const string CoreWorkload::ZIPFIAN_THETA_PROPERTY = "theta";
const string CoreWorkload::ZIPFIAN_THETA_DEFAULT = "0.99";

void CoreWorkload::InitLoadSharedState(const utils::Properties &p) {
    // key_generator_ (for load phase, and insertion in run phase)
    uint64_t insert_start = std::stoul(p.GetProperty(INSERT_START_PROPERTY,
                                                     INSERT_START_DEFAULT));
    AckedCounterGenerator::InitShared(insert_start);
}

void CoreWorkload::InitLoad(const utils::Properties &p) {
  ...
  key_generator_ = new AckedCounterGenerator();

  loaded_ = true;
}

void CoreWorkload::InitRunSharedState(const utils::Properties &p) {
    // key_generator_ (for load phase, and insertion in run phase)
    uint64_t record_count = std::stoul(p.GetProperty(RECORD_COUNT_PROPERTY));
    AckedCounterGenerator::InitShared(record_count);
}

void CoreWorkload::InitRun(const utils::Properties &p) {
  if(!loaded_) {
      // "run" only
      table_name_ = p.GetProperty(TABLENAME_PROPERTY,TABLENAME_DEFAULT);
  ...
  double zipfian_theta = std::stod(p.GetProperty(ZIPFIAN_THETA_PROPERTY,
                                                 ZIPFIAN_THETA_DEFAULT));
  ...
  if (request_dist == "uniform") {
    ...
    uint64_t op_count = std::stoul(p.GetProperty(OPERATION_COUNT_PROPERTY));
    uint64_t new_keys = (uint64_t)(op_count * insert_proportion * 2.0); // a fudge factor
    key_chooser_ = new ScrambledZipfianGenerator(record_count_ + new_keys, zipfian_theta);
  } else if (request_dist == "latest") {
    key_chooser_ = new SkewedLatestGenerator(key_generator_);
  } else {
