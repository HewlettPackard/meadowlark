...
// NOTE: current implementation is thread-safe only when Next() is being used
// When Next(uint64_t num) is used (e.g., by SkewedLatestGenerator ), it is NOT thread-safe
namespace ycsbc {
...
class ZipfianGenerator : public Generator<uint64_t> {
 ...
 private:
  ...
  double Eta() {
    return (1 - std::pow(2 / num_items_, 1 - theta_)) /
        (1 - zeta_2_ / zeta_n_);
  }
