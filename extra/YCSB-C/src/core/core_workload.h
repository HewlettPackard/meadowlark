...
class CoreWorkload {
 public:
  ...
  static const std::string ZIPFIAN_THETA_PROPERTY;
  static const std::string ZIPFIAN_THETA_DEFAULT;
    
  ///
  /// Initialize shared state
  /// Called once, in the main thread
  ///
  static void InitLoadSharedState(const utils::Properties &p);
  static void InitRunSharedState(const utils::Properties &p);

  ///
  /// Initialize per thread state
  /// Called once, in every worker thread, before any operations are started.
  ///
  virtual void InitLoad(const utils::Properties &p);
  virtual void InitRun(const utils::Properties &p);
  ...
  virtual std::string NextSequenceKey(uint64_t &num); /// Used for loading data
  virtual void AckSequenceKey(uint64_t num); /// 
  ...
  CoreWorkload() :
      loaded_(false),
      field_count_(0), ordered_inserts_(true), record_count_(0), field_len_generator_(NULL),
      read_all_fields_(false), write_all_fields_(false),
 ...
 protected:
  ...
  bool loaded_; // if the current thread loaded data or not
  ...
  AckedCounterGenerator *key_generator_; // always shared by threads/processes
  DiscreteGenerator<Operation> op_chooser_;
  Generator<uint64_t> *key_chooser_; // uses the key_generator_ when request_dist == "latest"
...
inline void CoreWorkload::AckSequenceKey(uint64_t key_num) {
  key_generator_->Ack(key_num);
}
